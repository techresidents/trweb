define([
    ...
    'text!./templates/filter_factory.html',
    'text!./templates/filter_factory_drop.html'
    ...
    var SelectFilterView = FilterView.extend({
        ...
        selectionCollectionToFilter: function() {
            ...
            if(values.length) {
                filter = new api.query.ApiQueryFilter({
                    name: this.field,
    ...
    SelectFilterView.Factory = core.factory.buildFactory(SelectFilterView);
    ...
    AutoSelectFilterView.Factory = core.factory.buildFactory(AutoSelectFilterView);
    ...
    var DateRangeFilterView = FilterView.extend({
        ...
        filterToDateRange: function(filter) {
            ...
            if(filter) {
                ...
                start = core.date.Date.fromTimestamp(parseFloat(values[0]));
                end = core.date.Date.fromTimestamp(parseFloat(values[1]));
                result = new core.date.DateRange(start, end);
            }
            return result;
        },
        ...
        dateRangeToFilter: function(range) {
            ...
            if(range && range.start && range.end) {
                start = new core.date.DateTime(range.start.date);
                end = new core.date.DateTime(range.end.date).add(
                        new core.date.Interval(0, 0, 0, 23, 59, 59));
                values = [start.getTimestamp(), end.getTimestamp()];
                result = new api.query.ApiQueryFilter({
                    name: this.field,
                    value: values.join(':'),
                    op: 'range'
                });
    ...
    DateRangeFilterView.Factory = core.factory.buildFactory(DateRangeFilterView);
     ...
     */
    var FilterFactoryView = core.view.View.extend({
        ...
        initChildViews: function() {
            var view = new core.factory.Factory(select_views.MultiSelectView, {
                collection: this.selectionCollection
            });
