...
env.project_site = "techresidents.com"
...
def deploy(version):
    ...
    with settings(user="root"):
        ...
        #to allow for the local testing of different projects without having to
        #modify the global apache configuration.
        if env.project_environment == "localdev":
...
def create_database():
    ...
    put(sqlfile)
    run("psql -U postgres -h 127.0.0.1 -f %s" % os.path.basename(sqlfile))
